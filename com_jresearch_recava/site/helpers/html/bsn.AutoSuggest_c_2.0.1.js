	...
	if (this.oP.json)
	{
		...
		if(jsondata.results.length >0 && jsondata.results[0].codigo != ''){
			for (var i=0;i<jsondata.results.length;i++)
			{
				...
				, 'teamid' : jsondata.results[i].teamid }  );
			}
		}else{
			for (var i=0;i<jsondata.results.length;i++)
			{
				this.aSuggestions.push(  { 'id':jsondata.results[i].id, 'value':jsondata.results[i].value, 'info':jsondata.results[i].info, 'teamid':jsondata.results[i].teamid }  );
			}
		}
	}
	else
	{
		...
		for (var i=0;i<results.length;i++)
		{
			if (results[i].hasChildNodes())
				this.aSuggestions.push(  { 'id':results[i].getAttribute('id'), 'value':results[i].childNodes[0].nodeValue, 'info':results[i].getAttribute('info'), 'teamid' : results[i].getAttribute('teamid') }  );
		}
...
_bsn.AutoSuggest.prototype.appendMember = function(isInternal){
	...
	if(isInternal){
		memberValue = this.aSuggestions[ this.iHighlighted-1 ].id;
		teamid = this.aSuggestions[ this.iHighlighted-1 ].teamid;
	}else{
		memberValue = this.fld.value;
	}
	hiddenInput.setAttribute('value', memberValue);

	//Get the teams master list
	var masterteamslist = document.getElementById('masterteamslist_'+this.fld.name);
	var masterbooleanlist = document.getElementById('masterbooleanlist_'+this.fld.name);

	var listcontrol = masterteamslist.cloneNode(true);
	var booleancontrol = masterbooleanlist.cloneNode(true);
	listcontrol.setAttribute('name', 'teams_' + this.fld.name + nResults);
	listcontrol.setAttribute('id', 'teams_' + this.fld.name + nResults);
	booleancontrol.setAttribute('name', 'boolean_'+ this.fld.name + nResults);
	booleancontrol.setAttribute('id', 'boolean_'+ this.fld.name + nResults);
	booleancontrol.setAttribute('onchange', 'javascript:toogleTeamsControl(' + nResults + ', \'' + this.fld.name + '\')' );
	booleancontrol.style.display = 'inline';

	if(teamid != '-1'){
		booleancontrol.selectedIndex = 0;
		listcontrol.value = teamid;
		listcontrol.style.display = 'inline';
	}
	
	booleanLabelSpan = document.createElement('span');
	booleanLabelSpan.style.padding = '2px';
	booleanLabelSpan.appendChild(document.createTextNode(this.lbl_belongs_recava));
	
	booleanSpan = document.createElement('span');
	booleanSpan.style.padding = '2px';
	booleanSpan.appendChild(booleancontrol);
	
	teamsSpan = document.createElement('span');
	teamsSpan.style.padding = '2px';
	teamsSpan.appendChild(listcontrol);
	...
	newLi.appendChild(booleanLabelSpan);
	newLi.appendChild(booleanSpan);
	newLi.appendChild(teamsSpan);
...
_bsn.AutoSuggest.prototype.setTeam = function(httpRequest){
	var xmlResponse = httpRequest.responseXML;
	var indexArray = xmlResponse.getElementsByTagName('index');
	var valueArray = xmlResponse.getElementyByTagName('value');
	
	var index = indexArray.items[0].firstChild.nodeValue;
	var value = indexArray.items[0].firstChild.nodeValue;
	
	var booleanControl = document.getElementById('boolean_'+this.fld.name+index);
	var teamsControl = document.getElementById('teams_'+this.fld.name+index);
	
	if(booleanControl && teamsControl){
		//Here do the mapping
		if(value != '-1'){
			teamsControl.selectedIndex = parseInt(value);
			booleanControl.value = 'yes';
		}		
	}
}
