<?php
/**
* @version		$Id$
* @package		J!Research
* @subpackage	Publications
* @copyright	Copyright (C) 2008 Luis Galarraga.
* @license		GNU/GPL
* Joomla! is free software. This version may have been modified pursuant
* to the GNU General Public License, and as distributed it includes or
* is derivative of works licensed under the GNU General Public License or
* other free or open source software licenses.
* See COPYRIGHT.php for copyright notices and details.
*/

defined( '_JEXEC' ) or die( 'Restricted access' );
require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'tables'.DS.'publication.php');

/**
 * The class JResearchBook is subclass of JResearchPublication and holds
 * the information of a printed book.
 *
 */
class JResearchBook extends JResearchPublication{
	
	/**
	 * Database integer ID
	 *
	 * @var int
	 */
	public $id_publication;
	
	/**
	 * Book's publisher
	 * 
	 * @var string
	 */
	public $publisher;
	
	/**
	 * Book's editors separated by commas.
	 * 
	 * @var string
	 */
	public $editor;
	
	/**
	 * Book's volume
	 *
	 * @var string
	 */
	public $volume;
	
	/**
	 * The number of the book when it is part of a series.
	 * 
	 * @var string
	 */
	public $number;
	
	
	/**
	 * Name of the series, the book is part of.
	 *
	 * @var string
	 */
	public $series;
	
	/**
	 * Usually, publisher's address
	 * 
	 * @var string
	 */
	public $address;
	
	
	/**
	 * Book's edition
	 *
	 * @var string
	 */
	public $edition;
	
	/**
	 * Month of publication
	 * 
	 * @var string
	 */
	public $month;
	

	/**
	 * Class constructor.  It maps the entity to Joomla tables
	 *
	 * @param JDatabase $db
	 */
	function __construct(&$db){
		parent::__construct($db);
		parent::setDerivedTable('#__jresearch_book');
		$this->pubtype = 'book';
	}
	
	
}

?>