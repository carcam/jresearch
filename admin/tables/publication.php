...
class JResearchPublication extends JResearchActivity{
    ...
    */
    ...
    public function store($updateNulls = false){
        ...
        $researchareaRemoveQuery = 'DELETE FROM '.$db->quoteName('#__jresearch_publication_research_area').' WHERE id_publication = '.$db->Quote($this->id);
        ...
        foreach($idsAreas as $area){
            $insertAreaQuery = 'INSERT INTO '.$db->quoteName('#__jresearch_publication_research_area').'(id_publication, id_research_area) VALUES('.$db->Quote($this->id).', '.$db->Quote($area).')';	
        ...
        $keywords = explode(JResearchActivity::$_keywordsDelimiter, trim($this->keywords));
    ...
    /**
     * Returns a list of associative arrays with the information about the 
     * attachments associated to the publication.
     * @return type
     */
    public function getAttachments() {
        return parent::getAttachmentsByController('publications');
    }    
    ...
    public function getAttachment($tag) {
        $this->getAttachments();
        foreach ($this->_attachmentsArray as $entry) {
            if ($entry['tag'] == $tag) {
                return $entry;
            }
        }
