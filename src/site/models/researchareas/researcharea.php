...
class JResearchModelResearchArea extends JResearchModelItem{
    ...
    function getLatestProjects($n = 0){
        ...
        $areaId = $row->id;
        $db = JFactory::getDBO();
        $query = 'SELECT * FROM '.$db->quoteName('#__jresearch_project').' WHERE '.$db->quoteName('published').' = 1'
    ...
    function countProjects(){
        ...
        $query = 'SELECT count(*) FROM '.$db->quoteName('#__jresearch_project').' WHERE '.$db->quoteName('published').' =  1'
                .' AND '.$db->quoteName('id_research_area').' = '.$db->Quote($areaId);
        $db->setQuery($query);
        return (int)$db->loadResult();
    }
