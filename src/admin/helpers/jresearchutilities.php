...
{	
    ...
    private static function _createThumbnail($uploadedFile, $folder) {
        ...
        //Create thumbnail
        if($imageUploaded && imagecopyresampled($imageThumb, 
                $imageUploaded, 0, 0, 0, 0, $thumbSize->width, 
                $thumbSize->height, $width, $height)) {
    ...
    /**
     * It returns a string encoding the information of the attachments stored in
     * the array sent as argument. The method assumes they were generated using
     * the J!Research attachment control under models/fields/attachment.php
     * @param type $data
     */
    public static function processAttachments(&$data, $controller) {
        ...
        for($i = 0; $i <= $nAttach; ++$i) {
            ...
            if($delete == 'on') {
                if (!self::isValidURL($data['old_files_'.$i])) {
                    $filetoremove = JRESEARCH_COMPONENT_ADMIN.DS.$params->get('files_root_path', 'files').DS.$controller.DS.$theFile;
                    @unlink($filetoremove);
                }
            }else{
